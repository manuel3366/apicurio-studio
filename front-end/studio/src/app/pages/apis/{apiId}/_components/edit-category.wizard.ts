...
@Component({
    selector: "edit-category-wizard",
    templateUrl: "edit-category.wizard.html",
    styleUrls: [ "edit-category.wizard.css" ]
})
export class editCategoryWizardComponent {

    @ViewChildren("editCategoryModal") editCategoryModal: QueryList<ModalDirective>;
    ...
    public open(): void {
        ...
        this._isOpen = true;
        this.editCategoryModal.changes.subscribe( thing => {
            if (this.editCategoryModal.first) {
                this.editCategoryModal.first.show();
            }
        });
        this.loading = true;
        ...
        this.linkedAcounts.getLinkedAccounts().then( accounts => {
            ...
            console.debug("[editCategoryWizardComponent] Linked accounts loaded.");
        }).catch(error => {
            console.error("[editCategoryWizardComponent] Error getting Linked Accounts");
            this.error = error;
        ...
        this.apis.getCodegenProjects(this.apiId).then( projects => {
            ...
            console.debug("[editCategoryWizardComponent] Codegen projects loaded.  still loading: ", this.loading);
        }).catch( error => {
            console.error("[editCategoryWizardComponent] Error getting Codegen Projects");
            this.error = error;
            this.errorMessage = "Error getting Wizard information.";
        });
        this.apis.getApi(this.apiId).then( api => {
          this.modelApi = api;
        }).catch( error => {
        });
    ...
    protected cancel(): void {
        this.editCategoryModal.first.hide();
    }
    ...
    public goNext(): void {
        this.apis.updateCategoryDesign(this.apiId, this.modelApi.category ).then( updateApiCategory => {
            this.close();
       }).catch( error => {
           this.error(error);
       });
